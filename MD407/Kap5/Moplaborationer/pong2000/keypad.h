    ...
    switch(row)
        {
            ...
            case 0: *GPIO_E_ODR_HIGH = 0x00; break;
        }
...
void outSeg7(unsigned char in)
    {
        ...
        if(in == 0x0) *GPIO_E_ODR_LOW = 0x3F;
        if(in == 0x1) *GPIO_E_ODR_LOW = 0x06;
        if(in == 0x2) *GPIO_E_ODR_LOW = 0x5B;
        if(in == 0x3) *GPIO_E_ODR_LOW = 0x4F;
        if(in == 0x4) *GPIO_E_ODR_LOW = 0x66;
        if(in == 0x5) *GPIO_E_ODR_LOW = 0x6D;
        if(in == 0x6) *GPIO_E_ODR_LOW = 0x7D;
        if(in == 0x7) *GPIO_E_ODR_LOW = 0x07;
        if(in == 0x8) *GPIO_E_ODR_LOW = 0x7F;
        if(in == 0x9) *GPIO_E_ODR_LOW = 0x67;
        if(in == 0xA) *GPIO_E_ODR_LOW = 0x77;
        if(in == 0xB) *GPIO_E_ODR_LOW = 0x7F;
        if(in == 0xC) *GPIO_E_ODR_LOW = 0xFF;
        if(in == 0xD) *GPIO_E_ODR_LOW = 0xBF;
        if(in == 0xE) *GPIO_E_ODR_LOW = 0x79;
        if(in == 0xF) *GPIO_E_ODR_LOW = 0x71;
	else *GPIO_E_ODR_LOW = 0x00;
